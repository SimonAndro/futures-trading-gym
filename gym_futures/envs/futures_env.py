...
class FuturesEnv(gym.Env):
    ...
    def __init__(self, states: Sequence[TimeSeriesState], value_per_tick: float,
        ...
        self.observation_space = states[0].feature_space()
    ...
    def buy(self, state: TimeSeriesState):
        ...
        if self.current_position == 1:
            ...
            self.exit_price = self.generate_random_fill_differential(
                state.price, 1)
            ...
            self.entry_price = self.generate_random_fill_differential(
                state.price, 1)
    ...
    def reset(self, e=0):
        ...
        self.episode = e
        return self.states[0].to_feature_vector(), {"message": "reseted"}
    ...
    def _close_position(self, reward: float, net_profit: float):
        ...
        if all([self.current_position == 0, self.last_position == -1]):
            # closed a short
            self.trades.append(
                [trade_id, "short", self.entry_id, self.exit_id, net_profit, reward, duration])
        elif all([self.current_position == 0, self.last_position == 1]):
            # closed a short
            self.trades.append(
                [trade_id, "long", self.entry_id, self.exit_id, net_profit, reward, duration])
    ...
    def get_episode_data(self, return_states: bool = False) -> Tuple[pandas.DataFrame, pandas.DataFrame]:
        ...
        trade_cols = ['trade_id', 'trade_type', 'entry_order_id',
                      'exit_order_id', 'profit', 'reward', 'duration']
    ...
    def generate_episode_metrics(self, as_file: bool = True) -> dict:
        ...
        l_win_pl = sum(long_df['profit']
                       [long_df['profit'] >= 0]) if l_wins > 0 else 0
        l_loss_pl = sum(long_df['profit']
                        [long_df['profit'] < 0]) if l_losses > 0 else 0
        l_max_win = np.max(
            long_df['profit'][long_df['profit'] >= 0]) if l_wins > 0 else 0
        l_max_loss = np.min(
            long_df['profit'][long_df['profit'] < 0]) if l_losses > 0 else 0
        l_profit_factor = abs(l_win_pl) / \
            abs(l_loss_pl) if l_loss_pl != 0 else 1.
        ...
        s_max_win = np.max(
            short_df['profit'][short_df['profit'] >= 0]) if s_wins > 0 else 0
        s_max_loss = np.min(
            short_df['profit'][short_df['profit'] < 0]) if s_losses > 0 else 0
        s_win_pl = sum(short_df['profit']
                       [short_df['profit'] >= 0]) if s_wins > 0 else 0
        s_loss_pl = sum(
            short_df['profit'][short_df['profit'] < 0]) if s_losses > 0 else 0
        s_win_avg = np.mean(
            short_df['profit'][short_df['profit'] >= 0]) if s_wins > 0 else 0
        s_loss_avg = np.mean(
            short_df['profit'][short_df['profit'] < 0]) if s_losses > 0 else 0
        s_profit_factor = abs(s_win_pl) / \
            abs(s_loss_pl) if s_loss_avg != 0 else 1.
        ...
        expectancy = (win_percentage * win_avg) - \
            ((1-win_percentage) * loss_avg)
    ...
    def _generate_episode_graphs(self):
        ...
        plt.savefig(
            f"{self.log_dir}/img/duration_distribution_episode_{self.episode}.png")
        plt.clf()
        plt.hist(long_df["profit"], bins=20, alpha=0.5, label="long")
        plt.hist(short_df["profit"], bins=20, alpha=0.5, label="short")
        plt.savefig(
            f"{self.log_dir}/img/profit_loss_distribution_episode_{self.episode}.png")
        plt.clf()
    ...
    def _get_next_state(self):
        ...
        if self.current_index <= self.limit - 1:
            ...
            if current_state.ts > next_state.ts:
                raise Exception(
                    "the time stamp of the current state is greater than the next state")
            # adds current position to the next state
            if self.add_current_position_to_state:
                next_state.set_current_position(
                    self.current_position, self.entry_time, self.entry_time)
